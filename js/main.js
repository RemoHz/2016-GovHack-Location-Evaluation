...
function nominati(event, index){
    var location = $("#address" + index).val().trim();

    if(location.length <= 5){
        return;
    }
    else{
        // Reconstrute address format
        var location = location.replace(" ", "+");

        var url = "https://nominatim.openstreetmap.org/search?q=" + location + "&countrycodes=au&accept-language=au&format=json&addressdetails=1&limit=5";
